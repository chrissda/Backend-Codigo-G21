...
class RegistroController(Resource):
    def post(self):
        ...
        try:
            ...
            # ahora reemplazamos el valor del password con el hasheo del password
...
class LoginController(Resource):
    def post(self):
        ...
        try:
            ...
            # el checkpw contrastara la password guardada en la bd con la password enviada en el login y si es, retornada True, caso contrario, False
            resultado = checkpw(password_en_bytes, password_bd_en_bytes)
            
            if resultado == True:
                # identity > es el identificador para reconocer a que usuario le pertenece ese token
...
class UsuarioController(Resource):
    # Este metodo get sera una ruta PROTEGIDA, es decir se necesitara una token de acceso para poder ingresar al controlador, caso contrario sera rechazada la token
    @jwt_required()
    def get(self):
    ...
    def put(self):
        ...
        # Modificar el nombre y el password mas no el correo
        # Crear un serializador MANUAL en el cual vamos a indicar que recibiremos el nombre y el password, ambos de caracter opcional
        # en base a la verificacion del serializador vamos a hacer la actualizacion de nuestro usuario
        ...
        try:
            ...
            # Si se hace el cambio del password entonces tenemos que generar su hash antes de guardarlo en la bd
        except ValidationError as error:
            return {
    ...
    def post(self):
        ...
        try:
            ...
            enviar_email(usuario_encontrado[1], 
                         'Restauracion de la contraseña', url)
            return {
                'message': 'Correo enviado con las indicaciones'
            }
        except ValidationError as error:
